...
namespace TechReturners.Tasks
{
    public class Exercise005
    {
        ...
        {
            var mexicanList = new List<string>();
            
            for (int i = 0; i < str.Length; i++)
            {
                if (!str.Substring(i, 1).Equals(" "))
                {
                    var audienceLeftSide = str.Substring(0, i).ToLower();
                    var waveIndividual = str.Substring(i, 1).ToUpper();

                    var audienceRightSide = str.Substring(i + 1);

                    mexicanList.Add(audienceLeftSide + waveIndividual + audienceRightSide);
                }
            }
            return mexicanList;
        }
